...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        # 不需要添加adapter的config
        self.shallow_config.has_adapter = False

        try:  # 查看是否设置高层adapters层数目
            self.adapter_nums = config.adapter_nums
        except:
            # 如果不曾设置，则默认adapters层数同transformers 层参数一致
            self.adapter_nums = config.num_hidden_layers

        # 不含adapters的层
        layer_list = [BertLayer(self.shallow_config) for _ in range(config.num_hidden_layers - self.adapter_nums)]
        # 含有adapters的层
        higher_list = [BertLayer(config) for _ in range(self.adapter_nums)]
        layer_list.extend(higher_list)
...
class BertAdapterForTokenClassification(ModelWithHeadsAdaptersMixin, BertPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
...
# class SkipBertAdapterForTokenClassification(ModelWithHeadsAdaptersMixin, BertPreTrainedModel):
#     _keys_to_ignore_on_load_unexpected = [r"pooler"]
#
#     def __init__(self, config, do_fit=False, share_param=True):
#         super().__init__(config)
